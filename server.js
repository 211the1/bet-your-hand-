...
function createServer(){const httpServer=http.createServer(serve);const wss=new WebSocketServer({server:httpServer});const sessions=new Map();
 wss.on('connection',ws=>{ws.isAlive=true;ws.on('pong',()=>{ws.isAlive=true});ws.on('message',async raw=>{try{await rooms.ready;const m=JSON.parse(raw),t=String(m.type||'').toUpperCase();if(t==='PING')return send(ws,{type:'PONG'});let s=sessions.get(ws),r;
  ...
  if(['PLAY_CARD','SPIN_WHEEL','USE_POWER','DRAW_CARD','CHOOSE_COLOR'].includes(t)){const g=r.game;if(!g)throw Error('Game has not started');if(t==='PLAY_CARD')e.playCard(g,s.playerId,m.cardId,{color:m.color});if(t==='SPIN_WHEEL')e.spinWheel(g,s.playerId);if(t==='USE_POWER')e.usePower(g,s.playerId,m.power,{color:m.color});if(t==='DRAW_CARD')e.drawCard(g,s.playerId);if(t==='CHOOSE_COLOR')e.chooseWildColor(g,s.playerId,m.color);await rooms.saveRoom(r.code);return rooms.sendState(r.code)}
