...
class RoomServer{
 ...
 async restartGame(c){const r=await this.getRoom(c);for(const [id,s] of r.sockets){if(id!==r.hostId){try{if(s.readyState===1)s.send(JSON.stringify({type:'ROOM_RESTARTED'}))}catch{}try{s.close(1000,'Room restarted by host')}catch{}}}r.players.clear();r.sockets=new Map();r.game=null;await this.saveRoom(r.code);const fresh=await this.createRoom(r.hostName);return fresh}
