...
class RoomServer{
 ...
 async joinRoom(c,name,character){
  const r=await this.getRoom(c);if(r.game)throw Error('Game already started');if(r.players.size>=6)throw Error('Room full');if(!String(name||'').trim())throw Error('Player name required');if(!engine.CHARACTERS.includes(character))throw Error('Invalid character');
  const id=`p${this.next++}`,p={id,name:String(name).trim(),character,token:token(),connected:false};r.players.set(id,p);await this.saveRoom(r.code);
  return{code:r.code,playerId:id,reconnectToken:p.token,host:false,name:p.name,character:p.character}
 }
