...
function executeGameAction(room, playerId, message) {
  ...
  if (type === 'PLAY_CARD') {
    ...
    result = { type: 'COLOR_CHOSEN', player: p, color: engine.currentColor(room.game) };
  } else if (type === 'SPIN_WHEEL') {
    // Random outcome is determined only by the server-side game engine.
    result = engine.spinPowerWheel(room.game, playerId);
  } else {
