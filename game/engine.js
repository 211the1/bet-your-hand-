...
function cardColor(card){if(!card)return null;if(card.type==='CHARACTER'){const c=String(card.id||'').split('-')[0];return COLORS.includes(c)?c:(COLORS.includes(card.color)?card.color:null)}if(card.type==='SKIP'||card.type==='REVERSE'){const n=Number(String(card.id||'').split('-')[1]);return Number.isFinite(n)?COLORS[n%4]:(COLORS.includes(card.color)?card.color:null)}return COLORS.includes(card.color)?card.color:null}
function currentColor(g){return g.currentColor||cardColor(topCard(g))||null}
function currentPlayer(g){return g.players[g.turnIndex]}
function isPlayable(card,g){if(card.type==='PLAY_YOUR_HAND'||card.type==='WILD')return true;const top=topCard(g),color=currentColor(g),cc=cardColor(card);return cc===color||(card.type==='CHARACTER'&&top?.character===card.character)}
