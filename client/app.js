...
function wheelHtml(result,spinning){const wedges=wheelSections.map((x,i)=>`<div class="wheel-label wheel-label-${x.color.toLowerCase()}" style="--i:${i}"><b>${x.section}</b><strong>${escapeHtml(x.character)}</strong><small>${escapeHtml(x.color)} • ${escapeHtml(x.power.replaceAll('_',' '))}</small></div>`).join('');const stops=wheelSections.map((x,i)=>{const start=i*40,end=start+40,c={Red:'#d91620',Blue:'#1687ff',Green:'#18b957',Yellow:'#e0ad16'}[x.color];return `${c} ${start}deg ${end}deg`}).join(',');const landed=Number(result?.section||0);const landRotation=landed>0?(1440-(landed-1)*40):0;const wheelClass=spinning?'wheel-spin':(result?'wheel-land':'');const wheelStyle=`--wheel-sectors:${stops};--wheel-land-rotation:${landRotation}deg`;return `<div class="wheel-box"><div class="wheel ${wheelClass}" style="${wheelStyle}"><div class="wheel-labels">${wedges}</div><div class="wheel-center"><b>POWER PLAY</b><small>BET YOUR HAND</small></div></div><div class="wheel-pointer">▼</div>${result?`<div class="wheel-result"><b>POWER PLAY: ${escapeHtml(result.power.replaceAll('_',' '))}</b><span>CHARACTER: ${escapeHtml(result.character||'—')}</span><span class="wheel-result-color">NEXT COLOR: ${escapeHtml(result.color||'—')}</span></div>`:`<div class="wheel-message">POWER WHEEL — SPINNING…</div>`}</div>`}
