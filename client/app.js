...
if(game.phase==='finished'){
 ...
 gameEl.innerHTML=`<div class="finish-screen" style="background-image:url('/finish-screen.png?v=1')"><div class="finish-screen-content"><div class="finish-screen-title">GAME NIGHT COMPLETE</div><div class="finish-screen-subtitle">WINNER</div><div class="finish-screen-winner">${escapeHtml(winner?.name||'—')}</div><div class="finish-screen-character">${escapeHtml(winner?.character||'')}</div><div class="finish-screen-scores">${scores.map((p,i)=>`<div class="finish-screen-row"><span>${i+1}. <b>${escapeHtml(p.name)}</b></span><span><b>${Number(p.points)}</b> points</span></div>`).join('')}</div><div class="finish-screen-round">ROUND 2 COMPLETE</div></div></div>`;
 return;
