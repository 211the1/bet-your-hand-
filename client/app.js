...
<div class="arcade-shell">
  <header class="arcade-header">
    ...
    <button id="back-to-join" class="arcade-logo" type="button">PLAY YOUR HAND</button>
...
ws.addEventListener('close',event=>{connecting=false;clearInterval(pingTimer);ws=null;if(event?.reason==='Room restarted by host'){joined=false;me=null;session=null;localStorage.removeItem('byhPlayerSession');showJoinScreen(false);setStatus('ROOM RESTARTED — ENTER THE NEW ROOM CODE',true);return}if(!joined){showJoinScreen();setStatus('CONNECTION LOST — ENTER ROOM CODE, NAME, AND CHARACTER',true)}else{setStatus('CONNECTION LOST — RETRYING…',true)}scheduleReconnect()})}
