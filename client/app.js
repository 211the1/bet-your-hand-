...
function wheelHtml(result,spinning){const wedges=wheelSections.map((x,i)=>`<div class="wheel-label wheel-label-${x.color.toLowerCase()}" style="--i:${i}"><b>${x.section}</b><strong>${escapeHtml(x.character)}</strong><small>${escapeHtml(x.power.replaceAll('_',' '))}</small></div>`).join('');const colorsCss=wheelSections.map(x=>({Red:'#d91620',Blue:'#1687ff',Green:'#18b957',Yellow:'#e0ad16'}[x.color])).join(',');const stops=wheelSections.map((x,i)=>{const start=i*40,end=start+40,c={Red:'#d91620',Blue:'#1687ff',Green:'#18b957',Yellow:'#e0ad16'}[x.color];return `${c} ${start}deg ${end}deg`}).join(',');return `<div class="wheel-box"><div class="wheel ${spinning?'wheel-spin':''}" style="--wheel-sectors:${stops}"><div class="wheel-labels">${wedges}</div><div class="wheel-center"><b>POWER PLAY</b><small>BET YOUR HAND</small></div></div><div class="wheel-pointer">▼</div>${result?`<div class="wheel-result"><b>POWER PLAY: ${escapeHtml(result.power.replaceAll('_',' '))}</b><span>COLOR: ${escapeHtml(result.color||'—')}</span><span>${escapeHtml(result.character||'')}</span></div>`:''}</div>`}
