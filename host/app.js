...
function renderHostFinishScreen(game){
  ...
  overlay.innerHTML='<div class="finish-player-overlay">'+scores.slice(0,5).map((p,i)=>{const img=({'Bug':'/Bug.jpg','Face':'/Face.jpg','Ling Ling':'/Ling_Ling.jpg','Beanz':'/Beanz.jpg','The One':'/The_One.jpg','Boone':'/Boone.jpg','Chicken Joe':'/Chicken_Joe.jpg','Juby':'/Juby.jpg','Meemaw':'/Meemaw.jpg'}[p.character]||'');return '<div class="finish-player finish-player-'+i+'">'+(img?'<img class="finish-player-photo" src="'+img+'" alt="">':'')+'<div class="finish-player-name">'+String(p.name||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</div><div class="finish-player-score">'+Number(p.points||0)+'</div></div>';}).join('')+'</div>';
